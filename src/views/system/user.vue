...
<template>
  <div>
    ...
    <el-table size="small"  :data="userData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      <el-table-column align="center" prop="machineCode" label="终端编号" width="120">
      </el-table-column>
      <el-table-column align="center"  prop="area" label="分区" width="120">
      </el-table-column>
      <el-table-column align="center"  prop="point" label="酒店名" width="120">
      </el-table-column>
      <el-table-column align="center"  prop="police" label="所属分局" min-width="50">
      </el-table-column>
      <el-table-column align="center"  prop="policePlace" label="所属派出所" min-width="50">
      </el-table-column>
      <el-table-column align="center"  prop="isPoi" label="是否有POI" min-width="50">
        ...
        <el-table-column align="center"  prop="poi" label="poi名称" min-width="120">
      </el-table-column>
       <el-table-column align="center"  prop="roomNum" label="房间数" min-width="50">
      </el-table-column>
       <el-table-column align="center"  prop="type" label="酒店类型" min-width="80">
            </el-table-column>
      <el-table-column align="center"  prop="address" label="酒店地址" width="200">
                  </el-table-column>
      ...
      <el-table-column align="center"  prop="updateTime" label="最后更新时间" min-width="120">
      </el-table-column>
      <el-table-column label="操作" align="center"  min-width="300">
        <template slot-scope="scope">
          ...
          <el-button v-if='!scope.row.isBusiness' size="mini" type="success" @click="work(scope.$index, scope.row,1)">营业</el-button>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="40%" @click='closeDialog("edit")'>
      <el-form label-width="120px" ref="editForm" :model="editForm">
        <el-form-item label="终端编号" v-if='showNewlyType=="xz"'>
          <el-input size="small" v-model="editForm.machineCode" auto-complete="off" placeholder="请输入终端编号"></el-input>
        </el-form-item>
    ...
    <el-dialog title="更换设备" :visible.sync="machineChangeVisible" width="30%" @click='machineChangeVisible=false'>
      <el-form label-width="80px" :model="machineChangeValue">
        <el-form-item label="设备号">
          <el-input size="small" v-model="machineChangeValue.machineCode" auto-complete="off" placeholder="请输入设备号"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click='machineChangeVisible=false'>取消</el-button>
        <el-button size="small" type="primary" :loading="loading" class="title" @click="machineChangeKeep">保存</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getData(){
      ...
      netWorkHttp('/list',this.formInline).then(res=>{
    ...
    // 编辑、添加提交方法
    submitForm() {
      let {machineCode,area,point,police,policePlace,isBusiness,id,isPoi,poi,type,roomNum,address} = this.editForm;
      if(machineCode&&area&&point&&police&&policePlace&&type&&roomNum&&address&&(isPoi==0||poi)){
        let data = {
          machineCode,
          area,
          point,
